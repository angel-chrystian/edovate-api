<cfcomponent accessors="true" displayname="TestHelper" hint="Provides different functions that return queries and to help with  testing">
  <cfproperty name="orgID" default="8450" />
	<cfproperty name="companyID" default="3536615" />
...
<cfscript>
  ...
  function getCompanyQuery( boolean top = 0, numeric minutesCached = 20 ){
    // Query parameters
    local.params = {
      orgID = { value = variables.orgID, cfsqltype="cf_sql_integer" }
    };
    ...
    savecontent variable="local.sql"{
      ...
      if( variables.companyID > 0 ){
        writeoutput("
          and en.pk_entityID = :companyID
        ");
        local.params.companyID = { value = variables.companyID, cfsqltype = "cf_sql_integer" };
      }
    }

    local.result = queryExecute( local.sql, local.params, { cachedWithin = createTimeSpan( 0, 0, arguments.minutesCached, 0 ) } );
    return local.result;
  }
</cfscript>
